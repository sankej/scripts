...
!(async () => {
  if ($.isNode()) {
    ...
    var fs = require('fs')
    soy_wx_jysz_token = fs.readFileSync('jsz-token.txt', 'utf8')
    if (soy_wx_jysz_token !== `undefined`) {
      soy_wx_jysz_token = soy_wx_jysz_token.split('\n')
    } else {
  ...
  for (i = 0; i < app_soy_wx_jysz_token.length; i++) {
    ...
    if (!soy_wx_jysz_User_Agent) {
      soy_wx_jysz_User_Agent =
        'Mozilla/5.0 (iPhone; CPU iPhone OS 12_5_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.13(0x18000d31) NetType/WIFI Language/zh_CN'
      soy_wx_jysz_headers = {
