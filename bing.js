...
async function all() {
  ...
  if (bingPointCookie == '') {
    ...
    if (dashBoard?.dashboard) {
      ...
      subtitle = `当前积分：${dashBoard?.dashboard?.userStatus?.availablePoints || '-'}${
        newPoint > 0 ? '+' + newPoint : ''
      }   日常获得：${dashBoard?.dashboard?.userStatus?.counters?.dailyPoint[0]?.pointProgress || '-'}/${
        dashBoard?.dashboard?.userStatus?.counters?.dailyPoint[0]?.pointProgressMax || '-'
      }`
    } else {
  ...
  return new (class {
    ...
    boxJsJsonBuilder(t, s) {
      if (this.isNode()) {
        ...
        o.icons = [
          `${e}/mini/master/Alpha/${this.id.toLocaleLowerCase()}.png`,
          `${e}/mini/master/Color/${this.id.toLocaleLowerCase()}.png`,
        ]
        o.keys = []
        o.settings = [
          {
    ...
    isEmpty(t) {
      return (
        typeof t == 'undefined' || t == null || t == '' || t == 'null' || t == 'undefined' || t.length === 0
      )
    }
    randomString(t) {
      t = t || 32
    ...
    formatDate(t, s) {
      ...
      for (let t in i)
        if (new RegExp('(' + t + ')').test(s))
          s = s.replace(RegExp.$1, RegExp.$1.length == 1 ? i[t] : ('00' + i[t]).substr(('' + i[t]).length))
      return s
    }
  })(t, s, i)
}
