...
function artshare(artsid) {
  return new Promise((resolve, reject) => {
    $.post(
      kdHost('WebApi/ShareNew/getShareArticleReward', cookie + '&' + 'article_id=' + artsid),
      async (error, resp, data) => {
        shareres = JSON.parse(data)
        if (shareres.status == 1) {
          $.log(
            '转发成功，共计转发' + shareres.data.items.share_num + '篇文章，获得青豆' + shareres.data.score,
          )
        }
        resolve()
      },
...
function rotaryCheck() {
  return new Promise(async (resolve) => {
    ...
    while (i <= 3) {
      if (
        100 - rotarytimes >= rotaryres.data.chestOpen[i].times &&
        rotaryres.data.chestOpen[i].received == 0
      ) {
        await runRotary(i + 1)
      }
      i++
    ...
    $.get(kdHost(`wap/user/balance?` + cookie), (error, response, data) => {
      ...
      if (infores.status == 0) {
        ...
        for (i = 0; i < infores.history[0].group.length; i++) {
          $.desc +=
            '【' + infores.history[0].group[i].name + '】' + infores.history[0].group[i].money + '个青豆\n'
        }
...
function GetCookie() {
  if ($request && $request.method != `OPTIONS` && $request.url.match(/\/NewTaskIos\/getTaskList/)) {
    ...
    signheaderVal =
      RefererVal.match(/&uid=\d+/) +
      RefererVal.match(/&cookie=[_a-zA-Z0-9-]+/) +
      RefererVal.match(/&cookie_id=[a-zA-Z0-9]+/)
      ...
      return Object(t) !== t
        ...
        : (Array.isArray(e) || (e = e.toString().match(/[^.[\]]+/g) || []),
          (e
            ...
            .reduce(
              (t, s, i) =>
                Object(t[s]) === t[s] ? t[s] : (t[s] = Math.abs(e[i + 1]) >> 0 == +e[i + 1] ? [] : {}),
              t,
            )[e[e.length - 1]] = s),
          t)
    }
