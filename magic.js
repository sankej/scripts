function MagicJS(e = 'MagicJS', t = 'INFO') {
  ...
  return new (class {
    ...
    notify(e = this.scriptName, t = '', s = '', i = '') {
      let o = (e) => {
        ...
        if (this.isSurge || this.isQuanX || this.isLoon) {
          if (typeof e === 'string') {
            if (this.isLoon) t = { openUrl: e }
            else if (this.isQuanX) t = { 'open-url': e }
            else if (this.isSurge) t = { url: e }
    ...
    log(e, t = 'INFO') {
      if (!(this.logLevels[this._logLevel] < this.logLevels[t.toUpperCase()]))
        console.log(`[${t}] [${this.scriptName}]\n${e}\n`)
    }
    logDebug(e) {
      this.log(e, 'DEBUG')
    }
    ...
    adapterHttpOptions(e, t) {
      ...
      if (!!!i.headers || typeof i.headers !== 'object' || !!!i.headers['User-Agent']) {
        ...
        if (this.isNode) i.headers['User-Agent'] = this.pcUserAgent
        else i.headers['User-Agent'] = this.iOSUserAgent
      }
      let o = false
      if (
        (typeof i['opts'] === 'object' &&
          (i['opts']['hints'] === true || i['opts']['Skip-Scripting'] === true)) ||
        (typeof i['headers'] === 'object' && i['headers']['X-Surge-Skip-Scripting'] === true)
      ) {
        o = true
      }
      if (!o) {
        if (this.isSurge) i.headers['X-Surge-Skip-Scripting'] = false
        else if (this.isLoon) i.headers['X-Requested-With'] = 'XMLHttpRequest'
        else if (this.isQuanX) {
      ...
      if (t === 'GET' && !this.isNode && !!i.body) {
        ...
        if (i.url.lastIndexOf('&') + 1 != i.url.length && i.url.lastIndexOf('?') + 1 != i.url.length)
          i.url += '&'
        i.url += e
        delete i.body
      }
    ...
    formatTime(e, t = 'yyyy-MM-dd hh:mm:ss') {
      ...
      for (let e in s)
        if (new RegExp('(' + e + ')').test(t))
          t = t.replace(RegExp.$1, RegExp.$1.length == 1 ? s[e] : ('00' + s[e]).substr(('' + s[e]).length))
      return t
    }
    now() {
